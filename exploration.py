...
class Combat:
    ...
    def choose_enemy(enemy_chosen):
        if enemy_chosen == None:
            ...
            # filters
            enemies_filtered_by_location = list(
                filter(
                    lambda x: entities.new_player.current_location
                    in entities.all_enemies[x].spawn_location,
                    entities.all_enemies,
                )
            )

            specific_enemy = list(
                filter(
                    lambda x: rdm_int in entities.all_enemies[x].spawn_range,
                    enemies_filtered_by_location,
                )
            )

            if len(specific_enemy) > 1:
                artifact_needed = list(
                    filter(
                        lambda x: x.location is entities.new_player.current_location,
                        all_artifacts,
                    )
                )[0]
        ...
        clear()
        print(
            f"{Colours.fg.cyan}You encountered {entities.new_player.current_enemy.name_string}{Colours.fg.cyan}."
        )
        sleep_and_clear(1)

    @staticmethod
    def set_effects():
        entities.new_player.current_enemy.current_health = (
            entities.new_player.current_enemy.max_health
        )
        ...
        for item_name in entities.new_player.items_used:
            ...
            if turns_left > 0:
                if turns_left == 1:
                    clear()
                    print(
                        f"{System.get_object(item_name, objects.all_items).name_string}{Colours.fg.orange}'s effects ran out."
                    )
                    sleep_and_clear(1.5)
    ...
    def display_items_used():
        filtered_items = list(
            filter(
                lambda item: entities.new_player.items_used[item] > 0,
        ...
        if len(filtered_items) > 0:
            print(
                f"{Colours.fg.lightgreen + Colours.underline}Current Item Effects:{Colours.reset}"
            )
        ...
        for index, filtered_item in enumerate(filtered_items):
            ...
            print(
                f"{Colours.tag(item_number)} {item_used.name_string}{Colours.equipment_colour}: {Colours.fg.red}({turns_left} turns left)"
            )

            objects.display_equipment_stats(
                item_used,
                display_price=False,
                display_name=False,
                extra_text=item_number,
            )
    ...
    def start_combat(cls, enemy=None, is_players_turn=None):
        ...
        if is_players_turn == None:
            if entities.new_player.has_combat_turn == None:
                entities.new_player.has_combat_turn = (
                    entities.new_player.armour.is_lighter_than(
                        entities.new_player.current_enemy.armour
                    )
                )
        else:
            if entities.new_player.has_combat_turn == None:
        ...
        while (
            entities.new_player.current_health > 0
            and entities.new_player.current_enemy.current_health > 0
        ):
            clear()
            ...
            if entities.new_player.has_combat_turn:
                if entities.new_player.combat_choice == None:
                    ...
                    while entities.new_player.combat_choice not in valid_inputs:
                        ...
                        entities.new_player.combat_choice = (
                            input(f"{Colours.input_colour}> ").lower().strip()
                        )
