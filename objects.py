...
class Object:
    def set_description(self, attributes_dict):
        ...
        updates_dict, entity, update_type = (
            updates["dict"],
            updates["name"],
            updates["update_type"],
        )
        ...
        for attribute in updates_dict:
            updated_attributes += word(entity, Colours.attribute_colour) + word(
                System.remove_unwanted_chars(attribute), Colours.attribute_colour
            )

            updated_by += word(
                updates_dict[attribute], Colours.attribute_colour
            ) + word("%", Colours.attribute_colour)
        ...
        if len(updated_attributes) > 1:
            string_to_add = (
                word(update_type) + updated_attributes + word("by") + updated_by
            )
    ...
    def __init__(self, name, price, affected_turns=0, increases={}, decreases={}):
        ...
        if self.affected_turns > 0:
            self.description.append(
                f"{Colours.fg.red}Affected Turns: {self.affected_turns}"
            )
...
purple_slush = Item(
    ...
    50,
    3,
    increases={"armour.defense": 10, "weapon.accuracy": 20, "weapon.crit_chance": 10},
)

kings_elixir = Item(
    "King's Elixir", 70, 2, increases={"armour.defense": 25, "weapon.accuracy": 50}
)
...
golems_breath = Item(
    ...
    200,
    2,
    increases={"weapon.accuracy": 50},
    decreases={"armour.defense": 40},
)


all_items = {
    "vlohg": vial_of_healing,
...
def display_equipment_stats(
    key, display_price=True, display_name=True, item_quantity="", extra_text=None
):
    if (
        key in entities.all_player_weapons
        or key in entities.all_player_armour
        or key in all_items
        or key in new_inventory.items_dict
    ):
    ...
    if display_price:
        price_string = (
            f"{space_to_display}{Colours.fg.yellow}Price: {specific_equipment.price}"
        )
    else:
        price_string = ""
    ...
    if isinstance(specific_equipment, entities.Weapon):
        print(
            f"""{key_to_display}{name_to_display}
...
def display_current_equipment_stats(category):
    ...
    print(
        f"{Colours.fg.green + Colours.underline}Your {category.capitalize()}:{Colours.reset}"
    )
        ...
        for key in self.items_dict:
            if self.items_dict[key] == [None, 0]:
                ...
                display_equipment_stats(
                    key, display_price=False, item_quantity=self.items_dict[key][1]
                )
...
"""
    ...
    def use_item(self):
        ...
        while not player_choice in self.items_dict and player_choice != "back":
            clear()
            print(
                f"""{Colours.fg.orange}Which item would you like to use?
...
{Colours.fg.green + Colours.underline}{cls.equipment_to_purchase.category.capitalize()} you want to buy:{Colours.reset}"""
    ...
    def handle_money(cls):
        ...
        if entities.new_player.gold_coins >= cls.total_price:
            if isinstance(cls.equipment_to_purchase, entities.Weapon) or isinstance(
            ...
            if isinstance(cls.equipment_to_purchase, entities.Weapon):
                ...
                new_inventory.add_item(
                    cls.key_of_equipment_to_purchase, cls.equipment_quantity
                )
    ...
    def handle_purchase(cls, category):
        ...
        # main shop loop
        while (
            first_player_choice not in valid_inputs_to_go_back
            and cls.has_made_purchase == False
        ):
            ...
            if first_player_choice in cls.equipment_dict:
                ...
                while (
                    second_player_choice not in valid_inputs_to_go_back
                    and cls.has_made_purchase == False
                ):
                    ...
                    second_player_choice = (
                        input(f"{Colours.fg.orange}> ").lower().strip()
                    )
