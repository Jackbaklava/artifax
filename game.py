...
What Would You Like To Do?{Colours.reset}"""
    ...
    def display_win_message():
        print(
            f"{Colours.fg.green + Colours.bold}Congratulations!!! You completed the game and successfully escaped from it."
        )
        input(f"{Colours.input_colour}> ")
    ...
    def display_screen_alert():
        print(
            f"{Colours.fg.yellow}NOTE: For the most fluid gameplay, please make sure that your screen/console size is set to max."
            + "\n"
        )
    ...
    def play(cls):
        while (
            not entities.new_player.is_dead()
            and not entities.new_player.current_enemy is entities.talgrog_the_giant
        ):
            if entities.new_player.main_menu_choice == None:
                # Display main menu
                cls.display_main_menu()
                entities.new_player.main_menu_choice = (
            ...
            # Player entered invalid choice
                ...
                clear()
                print(
                    f"{Colours.alert('INVALID COMMAND, TYPE THE LETTERS IN THE SQUARE BRACKETS.')}"
                )
                sleep_and_clear(2)
        ...
        if (
            entities.new_player.current_enemy is entities.talgrog_the_giant
            and not entities.new_player.has_won
        ):
            exploration.Combat.start_combat()
...
class GameState:
    ...
    def connect_account(cls):
        ...
        valid_inputs = {"1": cls.create_account, "2": cls.load_account}
        player_choice = ""

        while not cls.logged_in:
            clear()
            print(
                f"""{Colours.fg.orange + Colours.bold}
...
"""
    ...
    def create_account(cls):
        ...
        if (chosen_username, chosen_password) in cls.accounts_dict:
            clear()
            print(
                f"{Colours.fg.red}An account with the username '{chosen_username}' and password '{chosen_password}' already exists. Please Login."
            )
            sleep_and_clear(2)
            ...
            clear()
            print(
                f"{Colours.fg.red}Your password '{chosen_password}' doesn't match your confirmed password '{confirmed_password}'. Please try again."
            )
            sleep_and_clear(2)

        if (
            not (chosen_username, chosen_password) in cls.accounts_dict
            and chosen_password == confirmed_password
        ):
            cls.logged_in = True
